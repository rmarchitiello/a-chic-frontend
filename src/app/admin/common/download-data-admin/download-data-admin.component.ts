...
ngOnInit(): void {
  // Unisco tutti i media ricevuti in un array piatto (evito array annidati tipo MediaCloudinary[][])
  this.mediaInput = this.data.flatMap(item => item.media);

  // Log di controllo per verificare i file da scaricare
  console.log("[DownloadDataAdminComponent] oggetti ricevuti da scaricare: ", this.mediaInput);

  // Verifico se non ci sono file da scaricare
  this.checkDataIsEmpty = this.mediaInput.length === 0;

  // Se non ci sono file, chiudo il popup dopo 1 secondo
